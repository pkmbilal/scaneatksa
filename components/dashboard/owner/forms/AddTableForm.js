...
export default function AddTableForm({ restaurantId, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!n || n < 1 || n > 200) {
      setError(t("addTableForm.invalidCount"));
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setError(json?.error || t("addTableForm.failedGeneric"));
        setLoading(false);
        return;
      }
      ...
      setError(err?.message || t("addTableForm.serverError"));
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input
          ...
          placeholder={t("addTableForm.countPlaceholder")}
      ...
      <DialogFooter className="gap-2 sm:gap-0">
        ...
        >
          {loading ? t("addTableForm.submitting") : t("addTableForm.submit")}
        </button>
      </DialogFooter>
    </form>
  );
