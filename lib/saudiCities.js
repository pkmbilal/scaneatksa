...
// Legacy rows may hold a display name ("Riyadh", "Al Khobar") instead of the
// slug; normalise those so they still localise.
...
export function cityLabel(slug, locale) {
  if (!slug) return null
  const c = CITY_BY_SLUG[slug] || CITY_BY_SLUG[toSlug(slug)]
  if (!c) return slug
