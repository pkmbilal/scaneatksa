...
// reviews_insert_own policy). A review row is either restaurant-level
// (menu_item_id null) or scoped to one menu item. The public restaurant
// review list/section stays restaurant-level only, so it filters
// menu_item_id to null; owner-facing queries return both kinds together.
...
export async function getUserReviews() {
  ...
  const { data, error } = await supabase
    .from("reviews")
    .select("id, restaurant_id, order_id, menu_item_id, rating, comment, created_at, updated_at")
    .eq("user_id", user.id)
    .order("created_at", { ascending: false });
...
// Submit or edit a review -- either restaurant-level (menuItemId omitted) or
// scoped to one menu item from the order. Goes through the submit_review RPC
// rather than a plain upsert: the one-review-per-order dedup now lives in
// two partial unique indexes (one for the restaurant-level row, one for
// item-level rows keyed by menu_item_id too), and Postgres requires an
// explicit predicate in ON CONFLICT to target a partial index -- something
// PostgREST/supabase-js upsert({ onConflict }) can't express. The RPC picks
// the right ON CONFLICT target internally.
//
// reviewer_name is snapshotted (inside the RPC, from user_profiles) rather
// than joined at display time: reviews are publicly readable, but
// user_profiles RLS only lets a user read their own row
// (user_profiles_read_own), so a live join would silently show a blank name
// for every reviewer except the viewer themself. This mirrors how
// order_items already snapshots name/price off menu_items instead of
// joining live.
export async function submitReview({ restaurantId, orderId, menuItemId = null, rating, comment = null }) {
  const { data: au } = await supabase.auth.getUser();
  const user = au?.user;
  if (!user) return { data: null, error: { message: "Not logged in" } };
...
// Owner-side: reviews for the owner's own restaurant (both restaurant-level
// and item-level rows -- the menu_items(name) embed lets the UI label which
// review is about which dish) + reply CRUD. RLS (owns_restaurant) enforces
// ownership independently -- these are just the query shapes.
...
// Per-dish average rating for the owner's Reviews tab. menu_item_rating_summary
// doesn't carry item names (it's aggregated straight off reviews), so this
// fetches names in a second call rather than relying on PostgREST to embed
// menu_items through a view.
export async function getOwnerMenuItemRatings(restaurantId) {
  ...
  if (error || !summary?.length) return { data: [], error };
  ...
  return {
    data: summary.map((s) => ({
      ...
      name: nameById.get(s.menu_item_id) || null,
