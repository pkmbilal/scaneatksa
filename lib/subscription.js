...
// The most recent subscription_events row logged for a restaurant, or null if
// there is none. `events` doesn't need to be pre-sorted.
export function latestSubscriptionEvent(events, restaurantId) {
  let latest = null;
...
// if there is none. Used to notice a pending renewal request
// (action === "renewalRequested") -- it clears itself once any other action
// becomes the newest event.
export function latestSubscriptionAction(events, restaurantId) {
  return latestSubscriptionEvent(events, restaurantId)?.action || null;
}
